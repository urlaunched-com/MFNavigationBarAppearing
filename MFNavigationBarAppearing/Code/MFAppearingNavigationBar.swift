...
class MFAppearingNavigationBar: UINavigationBar {
    ...
    open private(set) var transparent = false
    
    open var titleInteractiveAppearing = true
    open var titleAppearingAnimated = false {
        didSet {
            titleInteractiveAppearing = !titleAppearingAnimated
        }
    }
    
    open var animationDuration: CGFloat = 0.33
...
private extension MFAppearingNavigationBar {
    ...
    func setupForNew(yOffset: CGFloat) {
        ...
        if yFullOffset < startTitleAppearingOffset {
            titleLabel!.frame = CGRect(x: titleFrame.minX, y: frame.height, width: titleFrame.width, height: titleFrame.height)
            
        } else if yFullOffset > endTitleAppearingOffset {
            titleLabel!.frame = CGRect(x: titleFrame.minX, y: minSpacing, width: titleFrame.width, height: titleFrame.height)
            
        } else if yFullOffset > startTitleAppearingOffset, yFullOffset < endTitleAppearingOffset {
            let currentDiff = yFullOffset - startTitleAppearingOffset
